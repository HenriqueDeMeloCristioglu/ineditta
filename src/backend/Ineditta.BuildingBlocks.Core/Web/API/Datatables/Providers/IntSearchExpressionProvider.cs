...
{
    public sealed class IntSearchExpressionProvider : DefaultSearchExpressionProvider
    {
            ...
            switch (op.ToLowerInvariant())
            {
                case EqualsOperator:
                    if (Nullable.GetUnderlyingType(left.Type) != null)
                    {
                        ...
                        return Expression.Equal(teste, Expression.Convert(right, typeof(int)));
                    }
                    return Expression.Equal(left, Expression.Convert(right, typeof(int)));
