...
namespace Ineditta.Repository.Migrations
{
    ...
    public partial class v117 : Migration
    {
        ...
        {
            migrationBuilder.Sql(@"ALTER TABLE documento_sindicato_mais_recente_usuario_tb ADD COLUMN id BIGINT AUTO_INCREMENT PRIMARY KEY FIRST;");

            migrationBuilder.Sql(@"alter table documento_sindicato_mais_recente_usuario_tb add (sindicato_patronal_id int)");
...
BEGIN
	                               ...
	                               DELETE FROM documento_sindicato_clausula_temp where usuario_id = usuario_id;
                                    ...
                                    json_table(dct.sind_patronal, '$[*]' columns (
                                        id int4 path '$.id',
                                        sigla varchar(500) path '$.sigla',
                                        codigo varchar(500) path '$.codigo'
                                    )) spmpt
                                   ...
                                   and YEAR(dct.data_liberacao_clausulas) > 1900;
                                    ...
                                    insert into documento_sindicato_mais_recente_usuario_tb
    	                                (sindicato_laboral_id, sindicato_patronal_id, ano_mes_validade_inicial, ano_mes_validade_final, documento_sindical_id, clausula_geral_id, estrutura_clausula_id, usuario_id, row_num)
   	                                select dstt.sindicato_laboral_id, dstt.sindicato_patronal_id, dstt.ano_mes_validade_inicial, dstt.ano_mes_validade_final, dstt.documento_sindical_id, cgt.clausula_id, cgt.estrutura_clausula_id, usuario_id, row_num                                   	
								...
								JOIN (
								    SELECT clausula_geral_id, MIN(id) AS menor_id
								    FROM documento_sindicato_mais_recente_usuario_tb dsmrut
								    WHERE dsmrut.usuario_id = usuario_id
								    GROUP BY clausula_geral_id
								    HAVING COUNT(1) > 1
								) x ON d1.clausula_geral_id = x.clausula_geral_id
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropPrimaryKey(
                name: "PRIMARY",
                table: "documento_sindicato_mais_recente_usuario_tb");
