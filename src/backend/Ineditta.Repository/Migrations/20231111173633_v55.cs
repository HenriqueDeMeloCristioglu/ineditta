        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"create or replace view comparativo_mapa_sindical_principal_vw as
                                   select dct.id_doc documento_id,
		                                  ...
		                                  coalesce(inpc.dado_real, 0) indice_projetado,
		                                  inpc.id inpc_id,
		                                  ...
		                                  dct.descricao_documento
   		                                ...
   		                                left join lateral(select idct.id,
   						                                idct.periodo_data, 
   						                                idct.dado_real  
   				                                     from indecon_real idct 
				                                     where idct.periodo_data = CONCAT(DATE_FORMAT((dct.validade_inicial - INTERVAL 1 MONTH), '%Y-%m'), '-01')
            ...
            migrationBuilder.Sql(@"CREATE OR REPLACE VIEW comparativo_mapa_sindical_item_vw AS
                                select
                                    ...
                                    coalesce(cgect.nome_informacao, 0) clausula_grupo_informacao_adicional_nome,
                                    (case
