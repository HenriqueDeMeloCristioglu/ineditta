	...
	keycloak
		.init({
			...
			initOptions: {
				...
				redirectUri: "http://localhost:8000/estruturaclausula.php", //a url da pagina que está implementando,
	...
	$.ajax(
		{
			...
			,dataType: "json"
			,data: {
				"module": "estruturaclausula", 
				"action": "addEstruturaClausula",
				"nome-inputc": $("#nome-inputc").val(),
                "cla-inputc": $("#cla-inputc").val(),
                "tipo-inputc": $("#tipo-inputc").val(),
                "classe-inputc": $("#classe-inputc").val(),
				"grupo-inputc": $("#grupo-inputc").val(),
				"assunto-inputc": $("#assunto-inputc").val()
...
function updateEstruturaClausula(id_estruturaclausula){
	...
	$.ajax(
		{
			...
			,data: {
				"module": "estruturaclausula", 
				"action": "updateEstruturaClausula",
				"nome-inputu": $("#nome-inputu").val(),
                "cla-inputu": $("#cla-inputu").val(),
                "tipo-inputu": $("#tipo-inputu").val(),
                "classe-inputu": $("#classe-inputu").val(),
				"grupo-inputu": $("#grupo-inputu").val(),
				"assunto-inputu": $("#assunto-inputu").val(),
...
$('#btn-atualizar').on('click', function() {
  ...
  updateEstruturaClausula(id);
});

function getByIdEstruturaClausula(id_estruturaclausula){
	...
	$.ajax(
		{
			...
			,data: {
				"module": "estruturaclausula", 
				"action": "getByIdEstruturaClausula",
				"id_estruturaclausula": id_estruturaclausula				
			}
			,beforeSend: function(xhr){
				//$('#preload').show();
			...
			,success: function (data) {
				...
				$("#nome-inputu").val( data.response_data.nome_grupo );
                $("#cla-inputu").val( data.response_data.nome_clausula);
                $("#tipo-inputu").val( data.response_data.tipo_clausula );
                $("#classe-inputu").val( data.response_data.classe_clausula );
				$("#grupo-inputu").val( data.response_data.grupo_calendario );
				$("#assunto-inputu").val( data.response_data.assunto_calendario );
