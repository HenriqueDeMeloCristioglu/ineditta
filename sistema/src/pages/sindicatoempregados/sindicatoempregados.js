...
import {
  ...
  AssociacaoService,
  UFService,
  CentralSindicalService,
  CnaeService,
  DataNegociacaoService,
  LocalizacaoService,
...
const centralSindicalService = new CentralSindicalService(apiService);
...
const sindicatoLaboralService = new SindicatoLaboralService(apiService, apiLegado);
...
const dataNegociacaoService = new DataNegociacaoService();

let ufSelect = null;
let grauSelect = null;
let statusSelect = null;
let dataNegociacaoSelect = null;
...
JQuery(async function () {
  ...
  await carregarSindicatoDatatable();
...
function configurarModal() {
  ...
  const centralSindicalModal = document.getElementById('centralSindicalModalHidden');
  const modalCentralSindicalContent = document.getElementById('centralSindicalModalHiddenContent');
  ...
  const modalsConfig = [
    {
      ...
      id: 'centralSindicalModal',
      modal_hidden: centralSindicalModal,
      content: modalCentralSindicalContent,
      btnsConfigs: [],
      onOpen: async () => await carregarCentralSindicalDatatable(),
      isInIndex: true
    }, {
...
function configurarFormulario() {
  ...
  dataNegociacaoSelect = new SelectWrapper("#data-negociacao-input", {options: { placeholder: 'Data negociação' }, onOpened: async () => await obterDatasNegociacao()});
  ...
  async function obterDatasNegociacao() {
    return await Promise.resolve(dataNegociacaoService.obterSelect());
  }
}
...
async function carregarBaseTerritorialSelecionadasDatatable() {
  ...
  baseTerritorialSelecionadasTable = new DataTableWrapper('#baseTerritoriaisSelecionadastb', {
    columns: [
      ...
      { "data": "dataNegociacao", title: "Data Negociação", render: (data) => dataNegociacaoService.converterNumeroDataNegociacao(data) }
    ],
    ...
    rowCallback: function (row, data, index) {
      ...
      if (basesTerritoriaisParaRemoverAdicacao.some(l => (l?.localizacaoId == data?.localizacaoId && l?.cnaeId == data?.cnaeId && l?.dataNegociacao == data?.dataNegociacao))) {
  ...
  async function obterBasesSelecionadasDt(request) {
    request.sindicatoLaboralId = $('#id-input').val() ?? 0;
    if (!baseTerritorialSelecionadasObtidasApi){
      const baseResult = await baseTerritorialSindicatoLaboralService.obterDatatable(request);
    ...
    if (request.Filter && request.Filter != ""){
      ...
      let basesTerritoriaisSelecionadasFiltradas = basesTerritoriaisSelecionadas?.filter(bt => 
        ...
        (bt?.dataNegociacao && dataNegociacaoService.converterNumeroDataNegociacao(bt?.dataNegociacao).toLowerCase().includes(filterValue))
      )
...
async function carregarCentralSindicalDatatable() {
  if (centralSindicalTable) {
    centralSindicalTable.reload();
    return;
  }

  centralSindicalTable = new DataTableWrapper('#centralSindicaltb', {
    columns: [
      { "data": "id" },
      { "data": "nome" },
      { "data": "sigla" },
    ...
    ajax: async (requestData) => {
      requestData.Columns = "id,nome,sigla,cnpj"
      return await centralSindicalService.obterDatatable(requestData)
    },
    rowCallback: function (row, data, index) {
      const button = $("<button>")
        .attr("data-id", data?.id)
      ...
      button.on("click", function () {
        const id = $(this).attr("data-id");
        const nome = $(this).attr("data-nome");
        inserirValorSelect('#central-sindical-input', id, nome);
      });
  ...
  await centralSindicalTable.initialize();
}
...
async function carregarBaseTerritorialHistoricoDatatable() {
  ...
  baseTerritorialHistoricoTable = new DataTableWrapper('#baseTerritorialHistoricotb', {
    columns: [
      { "data": "dataNegociacao", render: (data) => dataNegociacaoService.converterNumeroDataNegociacao(data) },
      { "data": "municipio" },
      { "data": "codigoUf" },
    ...
    ajax: async (requestData) => {
      requestData.Columns = "municipio,codigoUf,descricaoSubClasse,subclasseCnae,dataInicial,dataFinal";
      requestData.sindicatoLaboralId = $('#id-input').val();
      return await baseTerritorialSindicatoLaboralService.obterDatatable(requestData);
    }
  });
...
async function obterPorId(id) {
  ...
  const result = await sindicatoLaboralService.obterPorId(id);
  ...
  if (result.value.centralSindical){
    inserirValorSelect("#central-sindical-input", result.value.centralSindical.id, result.value.centralSindical.description)
  }
...
function limparFormulario() {
  ...
  basesTerritoriaisRemoverSelecionadas = [];
...
async function editar(id) {
  ...
  if (!$("#central-sindical-input").val() || $("#central-sindical-input").val() == "") {
    NotificationService.error({title: "Campo obrigatório vazio", message: "Central Sindical"});
    return;
  }
  ...
  const request = {
    ...
    "BasesTerritoriais": basesTerritoriaisSelecionadas.map(base => {
      return {baseTerritorialId: base.id, localizacaoId: base.localizacaoId, cnaeId: base.cnaeId, dataNegociacao: base.dataNegociacao}
    }),
    "centralSindicalId": $("#central-sindical-input").val(),
    "id": id
  };

  const result = await sindicatoLaboralService.editar(request);
...
async function incluir() {
  ...
  const result = await sindicatoLaboralService.incluir(request);
...
async function adicionarBasesTerritoriais() {
  ...
  if (!dataNegociacaoSelect?.getValue()) {
    NotificationService.error({title: "Campo obrigatório vazio", message: "Mês Negociação"});
    return;
  }
  ...
  localizacoesSelecionadas.forEach(loc => {
    cnaesSelecionados.forEach(cnae => {
      basesTerritoriaisSelecionadas.push({
        ...
        dataNegociacao: Number(dataNegociacaoSelect.getValue())
      });
    })
  })
  ...
  cnaesSelecionados = [];
  dataNegociacaoSelect.clear();

  let newBasesTerritoriaisSelecionadas = [];

  basesTerritoriaisSelecionadas.forEach((base) => {
    if (!newBasesTerritoriaisSelecionadas.some(bt => (bt.localizacaoId == base.localizacaoId && bt.cnaeId == base.cnaeId && bt.dataNegociacao == base.dataNegociacao))) {
...
async function removerBasesTerritoriaisParaAdicao() {
  ...
  basesTerritoriaisSelecionadas.forEach((base) => {
    if (!basesTerritoriaisParaRemoverAdicacao.some(bt => (bt.localizacaoId == base.localizacaoId && bt.cnaeId == base.cnaeId && bt.dataNegociacao == base.dataNegociacao))) {
      newBasesTerritoriaisSelecionadas.push(base);
    }
  });
  basesTerritoriaisSelecionadas = [...newBasesTerritoriaisSelecionadas];
