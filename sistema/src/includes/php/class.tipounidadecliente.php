...
class tipounidadecliente{
	...
	function getTipoUnidadeCliente( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdTipoUnidadeCliente( '.$obj->id_tiponegocio.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td class="title">';
						...
						$list .= '<td><button onclick="selectUnidadeClt('.$obj->id_tiponegocio.');" data-toggle="modal" href="#myModal" type="button" class="btn btn-secondary" data-dismiss="modal">Selecionar</button></td>';
						$list .= '<td class="title">';
						$list .= $obj->id_tiponegocio;
						$list .= '</td>';
						$list .= '<td class="desc">';
						$list .= $obj->tipo_negocio;
						$list .= '</td>';
						$list .= '</tr>';

						// $update .= '<tr class="odd gradeX">';
						$button .= '<button onclick="selectUnidadeClt('.$obj->id_tiponegocio.');" data-toggle="modal" href="#updateModal" type="button" class="btn btn-secondary" data-dismiss="modal">Selecionar</button>';
						// $update .= '<td>';
						// $update .= $obj->id_tiponegocio;
						// $update .= '</td>';
						// $update .= '<td>';
						// $update .= $obj->tipo_negocio;
						// $update .= '</td>';
						// $update .= '</tr>';
	...
	function getByIdTipoUnidadeCliente( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					$response['response_data']['id_tiponegocio'] 	= $obj->id_tiponegocio;
					$response['response_data']['tipo_negocio'] 	= $obj->tipo_negocio;
	...
	function addTipoUnidadeCliente( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into tipounidade_cliente
							(tipo_negocio)
						values
						('{$data['nome-input']}');
				";
	...
	function updateTipoUnidadeCliente( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE tipounidade_cliente
						SET  tipo_negocio = '{$data['nome-input']}'
								WHERE 
									id_tiponegocio = {$data['id_tiponegocio']}; 
						";
				$this->logger->debug( $sql );
