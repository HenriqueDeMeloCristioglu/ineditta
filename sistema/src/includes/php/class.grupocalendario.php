...
class grupocalendario{
	...
	function getGrupoCalendario( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdCalenDario( '.$obj->idgrupo_calendario.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
					
						$html .= '<td>';
						$html .= $obj->grupo_calendario;
						$html .= '</td>';
						$html .= '</tr>';
					}	
	...
	function getByIdCalenDario( $data = null ){
				...
				$sql = "
					SELECT 
						...
						FROM 
							grupo_calendario
						WHERE
                            idgrupo_calendario	 = {$data['idgrupo_calendario']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['idgrupo_calendario'] 	    = $obj->idgrupo_calendario;
					$response['response_data']['grupo_calendario'] = $obj->grupo_calendario;
	...
	function addGrupoCalendario( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into grupo_calendario
								(grupo_calendario)
							values
								('{$data['cal-inputc']}');
				";
	...
	function updateGrupoCalendario( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE   grupo_calendario
									SET  grupo_calendario = '{$data['cal-inputu']}'
									WHERE 
										idgrupo_calendario = {$data['idgrupo_calendario']};
						";
				$this->logger->debug( $sql );
