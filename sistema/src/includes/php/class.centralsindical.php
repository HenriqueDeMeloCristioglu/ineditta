...
class centralsindical{
	...
	function getCentralSindical( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td class="col-md-1 col-sm-1 col-xs-1"><a data-toggle="modal" href="#updateModal" onclick="getByIdCentralSindical( '.$obj->idcentralsindical_emp.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td class="col-md-8 col-sm-8 col-xs-8">';
						$html .= $obj->central_sindical;
						$html .= '</td>';
						$html .= '</tr>';
	...
	function getByIdCentralSindical( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				SELECT 
						...
						FROM 
							centralsindical_emp
						WHERE
							idcentralsindical_emp = {$data['idcentralsindical_emp']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['idcentralsindical_emp'] 	= $obj->idcentralsindical_emp;
					$response['response_data']['central_sindical'] 	= $obj->central_sindical;
	...
	function addCentralSindical( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into centralsindical_emp
							(central_sindical)
						values
						('{$data['nome-input']}');
				";
	...
	function updateCentralSindical( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE centralsindical_emp
						SET  central_sindical = '{$data['nome-input']}'
								WHERE 
									idcentralsindical_emp = {$data['idcentralsindical_emp']}; 
						";
				$this->logger->debug( $sql );
