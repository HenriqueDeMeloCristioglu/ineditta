...
class acompanhamento{
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdAcompanhamento( '.$obj->idcnae_sinde.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td>';
						$html .= $obj->idcnae_sinde;
						$html .= '</td>';
						$html .= '</tr>';
			...
			if( $response['response_status']['status'] == 1 ){
				...
				SELECT 
						...
						FROM 
							cnae_sinde
						WHERE
							idcnae_sinde = {$data['idcnae_sinde']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['idcnae_sinde'] 	= $obj->idcnae_sinde;
	...
	function updateAcompanhamento( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE cnae_sinde
						SET  idcnae_sinde = '{$data['num-input']}'
								WHERE 
									idcnae_sinde = {$data['idcnae_sinde']}; 
						";
				$this->logger->debug( $sql );
