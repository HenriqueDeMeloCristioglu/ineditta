...
class confederacaoemp{
	...
	function getConfederacaoEmp( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdConfederacaoEmp( '.$obj->id_confemp.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td>';
						$html .= $obj->id_confemp;
						$html .= '</td>';
						$html .= '<td>';
	...
	function getByIdConfederacaoEmp( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				SELECT 
						...
						FROM 
							confederacao_emp
						WHERE
							id_confemp = {$data['id_confemp']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['id_confemp'] 	= $obj->id_confemp;
					$response['response_data']['confemp'] 	= $obj->confemp;
	...
	function addConfederacaoEmp( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into confederacao_emp
							(confemp)
						values
						('{$data['nome-input']}');
				";
	...
	function updateConfederacaoEmp( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE confederacao_emp
						SET  confemp = '{$data['nome-input']}'
								WHERE 
									id_confemp = {$data['id_confemp']}; 
						";
				$this->logger->debug( $sql );
