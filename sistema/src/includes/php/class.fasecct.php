...
class fasecct{
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdFaseCct( '.$obj->id_fase.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td class="title">';
						$html .= $obj->fase_negociacao;
						$html .= '</td>';
						$html .= '<td class="desc">';
						$html .= $obj->prioridade;
						$html .= '</td>';
						$html .= '<td>';
						$html .= $obj->periodicidade;
						$html .= '</td>';
						$html .= '</tr>';
	...
	function getByIdFaseCct( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					$response['response_data']['id_fase'] 	= $obj->id_fase;
					$response['response_data']['fase_negociacao'] 	= $obj->fase_negociacao;
					$response['response_data']['prioridade'] 	= $obj->prioridade;
	...
	function addFaseCct( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into fase_cct
							(fase_negociacao
							,prioridade
							,periodicidade)
						values
						('{$data['fase-input']}', '{$data['pri-input']}',
						'{$data['per-input']}');
				";
				$this->logger->debug( $sql );
	...
	function updateFaseCct( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE fase_cct
						SET  fase_negociacao = '{$data['fase-input']}'
						,prioridade = '{$data['pri-input']}'
						,periodicidade = '{$data['per-input']}'
								WHERE 
									id_fase = {$data['id_fase']}; 
						";
				$this->logger->debug( $sql );
