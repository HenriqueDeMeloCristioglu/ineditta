		...
		if ($this->response['response_status']['status'] == 1) {
			...
			if ($response['response_status']['status'] == 1) {
				...
				if (true) {
					...
					$response['response_data']['listaPrincipal'] 	= "";
		...
		if ($this->response['response_status']['status'] == 1) {
			...
			if ($response['response_status']['status'] == 1) {
				...
				if ($data['tipo_doc'] == 'geral') {
					...
					<tbody>";
						while ($obj = $resultsql->fetch_object()) {
							...
							$html .= '<td><input type="checkbox" onclick="selectDoc( \'' . $obj->id_doc . '\',\'' . $obj->caminho_arquivo . '\');" id="check' . $obj->id_doc . '"></td>';
							$html .= '<td>' . $obj->nome_doc . '</td>';
							$html .= '<td>' . $obj->sigla_sinde . '</td>';
							$html .= '<td>' . $obj->inclusao . '</td>';
							$html .= '<td>' . $obj->vigencia . '</td>';
				...
				FROM doc_sind as doc 
					left join tipo_doc as td on td.idtipo_doc = doc.tipo_doc_idtipo_doc
					left join estrutura_clausula as ec on JSON_CONTAINS(doc.referencia, CONCAT('\"',ec.id_estruturaclausula, '\"'),'$')
				WHERE doc.modulo like 'SISAP' and doc.data_aprovacao IS NOT NULL and 
			...
			if ($response['response_status']['status'] == 1) {
				...
				if ($resultsql = mysqli_query($this->db, $sql)) {
					...
					while ($obj = $resultsql->fetch_object()) {
						...
						$select .= '<option value="' . $obj->id_consulta_documentos . '">' . $obj->data . '</option>';
			...
			if ($response['response_status']['status'] == 1) {
				...
				$sql = "SELECT
						...
						FROM
							consulta_documentos
						WHERE
						id_consulta_documentos = '{$data['id_consulta_documentos']}'
				...
				if ($resultsql = mysqli_query($this->db, $sql)) {
					...
					while ($obj = $resultsql->fetch_object()) {
						...
						$sqlReal = "SELECT * FROM consulta_documentos_real WHERE periodo = '{$obj->periodo}' AND indicador = '{$obj->indicador}' ";
		...
		if ($this->response['response_status']['status'] == 1) {
			...
			if ($response['response_status']['status'] == 1) {
				...
				if ($lineOne) {
					...
					$sql = "INSERT INTO
							consulta_documentos (indicador, id_usuario,  origem, fonte, data, dado_projetado, cliente_matriz_id_empresa)
						VALUES
		...
		if ($this->response['response_status']['status'] == 1) {
			...
			if ($response['response_status']['status'] == 1) {
				...
				if ($lineOne) {
					...
					$sql = "INSERT INTO
							consulta_documentos_real (indicador, periodo, dado_real)
						VALUES
							('{$data['indicador-input']}',
							'{$lineOne[0]}',
							'{$lineOne[1]}')
