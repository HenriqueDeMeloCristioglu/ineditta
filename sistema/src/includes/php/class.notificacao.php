		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				while ($obj = $resultList->fetch_object()) {
					...
					$user = (new usuario())->validateUser($_SESSION['login'])['response_data']['user'];

					if ($user->tipo = "Ineditta") {
						if ($obj->tipo_comentario == "filial") {
							$tipo = "Filial";
						}
					}else {
						break;
					...
					$html .= '</td>';
					$html .= '<td>';
					$html .= ($obj->usuario_adm_id_user == "" ? "--" : $obj->nome_usuario); // $obj->usuario_adm_id_user . " - " .
					$html .= '</td>';
					$html .= '<td>';
					$html .= $obj->comentario;
					$html .= '</td>';
				...
				SELECT 
                            ...
                            ,DATE_FORMAT(cm.data_inclusao,'%Y-%m-%d') AS data_inclusao
							,DATE_FORMAT(cm.data_inativacao,'%Y-%m-%d') AS data_inativacao
				...
				$sql = "SELECT 
                            ...
                            ,DATE_FORMAT(cu.data_inclusao,'%Y-%m-%d') AS data_inclusao
                            ,cm.nome_empresa as nome_empresa
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					}else if($obj->tipo_comentario == "filial") {
						$sql2 = "SELECT
									id_unidade,
									nome_unidade
									
								FROM cliente_unidades
								WHERE id_unidade = {$obj->id_tipo_comentario}
					...
					$response['response_data']['etiqueta'] = $obj->etiqueta;
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				$sql = "SELECT 
							...
							,cg.aprovado
							,date_format(data_aprovacao, '%Y-%m-%d %H:%i') as data_aprovacao
						FROM clausula_geral as cg 
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( !mysqli_query( $this->db, $sql )  ){
					...
					$response['response_status']['erro']         = $this->db->error;
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			// if( $response['response_status']['status'] == 1 ){
				...
				if( !mysqli_query( $this->db, $sql ) ){
					...
					$response['response_status']['dados']        = $data;
	...
	function getTipoComentario( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if ($data['tipo_comentario'] == "clausula") {
				...
				}else if($data['tipo_comentario'] == "filial") {
					$sql = "SELECT
								id_unidade,
								nome_unidade
					...
					while ($obj = $result->fetch_object()) {
						$opt .= $obj->nome_unidade != '-' ? "<option value='{$obj->id_unidade}'>{$obj->nome_unidade}</option>" : "";
					}
				}else {
					$sql = "SELECT
								id_sindp,
		...
		if ($this->response['response_status']['status'] == 1) {
			...
			if ($response['response_status']['status'] == 1) {
				...
				$sql = "SELECT 
						...
						INNER JOIN cliente_matriz cm ON cm.id_empresa = cu.cliente_matriz_id_empresa
						INNER JOIN tipounidade_cliente tuc on tuc.id_tiponegocio = cu.tipounidade_cliente_id_tiponegocio
						WHERE cm.cliente_grupo_id_grupo_economico = {$gecc};
				";
