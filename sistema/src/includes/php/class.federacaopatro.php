...
class federacaopatro{
	...
	function getFederacaoPatro( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdFederacaoP( '.$obj->id_fedpatro.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td>';
						$html .= $obj->id_fedpatro;
						$html .= '</td>';
						$html .= '<td>';
						$html .= $obj->nome_fedpatro;
						$html .= '</td>';
						$html .= '</tr>';
					}	
	...
	function getByIdFederacaoP( $data = null ){
				...
				$sql = "
					SELECT 
						...
						FROM 
							federacao_patro
						WHERE
                            id_fedpatro	 = {$data['id_fedpatro']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['id_fedpatro'] 	    = $obj->id_fedpatro;
					$response['response_data']['nome_fedpatro'] = $obj->nome_fedpatro;
	...
	function addFedePatro( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into federacao_patro
								(nome_fedpatro)
							values
								('{$data['patro-inputc']}');
				";
	...
	function updateFedePatro( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE federacao_patro  
									SET  nome_fedpatro = '{$data['patro-inputu']}'
									WHERE 
										id_fedpatro = {$data['id_fedpatro']};
						";
				$this->logger->debug( $sql );
