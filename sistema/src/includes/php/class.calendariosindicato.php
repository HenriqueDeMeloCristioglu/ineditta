...
class calendariosindicato{
	...
	function getCalendarioSindicato( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				SELECT 
					...
					,clausula
					,assunto_calendario
					,DATE_FORMAT(data_ocorrencia,'%d/%m/%Y')  AS data_ocorrencia
					,recorrencia
				FROM 
					calendario_sind;					
				";
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdCalendarioSindicato( '.$obj->id_calendariosind.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td>';
						$html .= $obj->clausula;
						$html .= '</td>';
						$html .= '<td>';
						$html .= $obj->assunto_calendario;
						$html .= '</td>';
						$html .= '<td>';
						$html .= $obj->data_ocorrencia;
						$html .= '</td>';
						$html .= '<td>';
						$html .= $obj->recorrencia;
						$html .= '</td>';
						$html .= '</tr>';
	...
	function getByIdCalendarioSindicato( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				FROM 
					calendario_sind
				WHERE
					id_calendariosind = {$data['id_calendariosind']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['id_calendariosind'] 	= $obj->id_calendariosind;
					$response['response_data']['clausula'] 	= $obj->clausula;
					$response['response_data']['assunto_calendario'] 	= $obj->assunto_calendario;
					$response['response_data']['data_ocorrencia'] 	= $obj->data_ocorrencia;
					$response['response_data']['recorrencia'] 	= $obj->recorrencia;
	...
	function addCalendarioSindicato( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into calendario_sind
							(clausula
							,assunto_calendario
							,data_ocorrencia
							,recorrencia)
						values
						('{$data['clau-input']}', '{$data['ass-input']}',
						STR_TO_DATE('{$data['data-input']}', '%d/%m/%Y'), '{$data['rec-input']}');
				";
				$this->logger->debug( $sql );
	...
	function updateCalendarioSindicato( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "  UPDATE calendario_sind 
								SET  clausula = '{$data['clau-input']}'
						,assunto_calendario = '{$data['ass-input']}'
						,data_ocorrencia = STR_TO_DATE('{$data['data-input']}', '%d/%m/%Y')
						,recorrencia = '{$data['rec-input']}'
								WHERE 
									id_calendariosind = {$data['id_calendariosind']};
						";
				$this->logger->debug( $sql );
