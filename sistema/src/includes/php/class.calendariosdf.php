...
class calendariosdf{
	...
	function getCalendarioSdf( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdCalendarioSdf( '.$obj->idcalendário_sdf.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td>';
						$html .= $obj->nome_sdf;
						$html .= '</td>';
						$html .= '<td>';
	...
	function getByIdCalendarioSdf( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				SELECT 
						...
						FROM 
							calendario_sdf
						WHERE
							idcalendário_sdf = {$data['idcalendário_sdf']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['idcalendário_sdf'] 	= $obj->idcalendário_sdf;
					$response['response_data']['nome_sdf'] 	= $obj->nome_sdf;
					$response['response_data']['abrang_sdf'] 	= $obj->abrang_sdf;
					$response['response_data']['data'] 	= $obj->data;
	...
	function addCalendarioSdf( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into calendario_sdf
							(nome_sdf
							,abrang_sdf
							,data)
						values
						('{$data['nome-input']}', '{$data['abr-input']}',
	...
	function updateCalendarioSdf( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE calendario_sdf
						SET  nome_sdf = '{$data['nome-input']}'
						,abrang_sdf = '{$data['abr-input']}'
						,data = STR_TO_DATE('{$data['data-input']}', '%d/%m/%Y')
								WHERE 
									idcalendário_sdf = {$data['idcalendário_sdf']}; 
						";
				$this->logger->debug( $sql );
