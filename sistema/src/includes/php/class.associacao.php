...
class associacao{
	...
	function getAssociacao( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdAssociacao( '.$obj->id_associacao.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td class="title">';
						$html .= $obj->sigla;
						$html .= '</td>';
	...
	function getByIdAssociacao( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					$response['response_data']['id_associacao'] 	= $obj->id_associacao;
					...
					$response['response_data']['nome'] 	= $obj->nome;
					$response['response_data']['area_geoeconomica'] = $obj->area_geoeconomica;
					$response['response_data']['grupo'] 	= $obj->grupo;
					$response['response_data']['classe'] 	= $obj->classe;
					$response['response_data']['categoria'] 	= $obj->categoria;
					...
					$response['response_data']['ramal'] 	= $obj->ramal;
	...
	function addAssociacao( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into associacao
						...
						values
							...
							'{$data['grau-input']}','{$data['nome-input']}','{$data['area-input']}',
							'{$data['gp-input']}', '{$data['cla-input']}','{$data['cat-input']}',
							...
							'{$data['ram-input']}','{$data['site-input']}','{$data['tw-input']}',
	...
	function updateAssociacao( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE associacao
						SET  sigla = '{$data['sigla-input']}'
							,cnpj = '{$data['cnpj-input']}'
							,status = '{$data['sta-input']}'
							,grau = '{$data['grau-input']}'
							,nome = '{$data['nome-input']}'
							,area_geoeconomica = '{$data['area-input']}'
							,grupo = '{$data['gp-input']}'
							,classe = '{$data['cla-input']}'
							,categoria = '{$data['cat-input']}'
