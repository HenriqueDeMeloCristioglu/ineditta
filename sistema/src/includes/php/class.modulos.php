...
class modulos{
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdMoDu( '.$obj->id_modulos.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
	...
	function getByIdMoDu( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					$response['response_data']['id_modulos'] 	= $obj->id_modulos;
					$response['response_data']['modulos'] 	= $obj->modulos;
					$response['response_data']['tipo'] 	= $obj->tipo;
					$response['response_data']['name_file_view'] 	= $obj->uri;
					$response['response_data']['criar_view'] 	= $obj->criar;
					$response['response_data']['consultar_view'] 	= $obj->consultar;
					$response['response_data']['comentar_view'] 	= $obj->comentar;
					$response['response_data']['alterar_view'] 	= $obj->alterar;
					$response['response_data']['excluir_view'] 	= $obj->excluir;
					$response['response_data']['aprovar_view'] 	= $obj->aprovar;
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				$sql = "insert into modulos
								(modulos, tipo, criar, consultar, comentar, alterar, excluir, aprovar, uri)
							values
								('{$data['mod-inputc']}', '{$data['tipo-input']}', '{$data['check-insert']}', '{$data['check-select']}', '{$data['check-comentario']}', '{$data['check-update']}', '{$data['check-delete']}', '{$data['check-aprovar']}', '{$data['name-file']}');
				";
				$this->logger->debug( $sql );
	...
	function updateModulos( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE modulos  
									SET  modulos = '{$data['mod-inputu']}', uri = '{$data['name-file-update']}', criar = '{$data['check-insert-update']}', consultar = '{$data['check-select-update']}', comentar = '{$data['check-comentario-update']}', alterar = '{$data['check-update-update']}', excluir = '{$data['check-delete-update']}', aprovar = '{$data['check-aprovar-update']}'
									WHERE 
										id_modulos = {$data['id_modulos']};
						";
				$this->logger->debug( $sql );
