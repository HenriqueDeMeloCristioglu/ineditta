...
class federacaoempregados{
	...
	function getFederacaoEmpregados( $data = null ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					...
					while($obj = $resultsql->fetch_object()){
						...
						$html .= '<td><a data-toggle="modal" href="#updateModal" onclick="getByIdFederacaoEmpregados( '.$obj->id_fedemp.');" class="btn-default-alt"  id="bootbox-demo-5"><i class="fa fa-file-text"></i></a></td>';
						$html .= '<td>';
						$html .= $obj->id_fedemp;
						$html .= '</td>';
						$html .= '<td>';
						$html .= $obj->nome_fedemp;
						$html .= '</td>';
						$html .= '</tr>';
	...
	function getByIdFederacaoEmpregados( $data = null ){
		...
		if( $this->response['response_status']['status'] == 1 ){
			...
			if( $response['response_status']['status'] == 1 ){
				...
				FROM 
					federacao_emp
				WHERE
					id_fedemp = {$data['id_fedemp']};
				";
				
				$this->logger->debug(  $sql );
				if( $resultsql = mysqli_query( $this->db, $sql ) ){
					$obj = $resultsql->fetch_object();
					$response['response_data']['id_fedemp'] 	= $obj->id_fedemp;
					$response['response_data']['nome_fedemp'] 	= $obj->nome_fedemp;
	...
	function addFederacaoEmpregados( $data = null ){
	...
	function updateFederacaoEmpregados( $data = null ){
		$this->logger->debug(  'entrou na classe php' );
		if( $this->response['response_status']['status'] == 1 ){
			...
			$this->logger->debug(  $connectdb );
			if( $response['response_status']['status'] == 1 ){
				$sql = " UPDATE federacao_emp  
									SET  nome_fedemp = '{$data['nome-input']}'
									WHERE 
										id_fedemp = {$data['id_fedemp']};
						";
				$this->logger->debug( $sql );
