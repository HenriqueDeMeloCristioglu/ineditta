			...
			if( $response['response_status']['status'] == 1 ){
				...
				$sql = "
					SELECT 
							...
							WHERE 
								cdtipoinformacaoadicional = {$codcliente};							
				";
				
				//$this->logger->debug(  $sql );

				$busca = 'Não localizado';

				if( $resultsql = mysqli_query( $this->db, $sql ) ){

					while($obj = $resultsql->fetch_object()){

						$busca = $obj->nmtipoinformacaoadicional;

					}	

					$response['response_data']['busca'] 	= $busca;
